...
def maybe_download_and_extract(hypes):
    ...
    crack500_dir = os.path.join(data_dir, 'data_CRACK500')

    if os.path.exists(vgg_weights):
        print("Got the vgg weights")

    if os.path.exists(crack500_dir):
        print("Got all the data")

    if os.path.exists(vgg_weights) and os.path.exists(crack500_dir):
        return
    ...
    # Extract and prepare CRACK500 DATA
    logging.info("Extracting CRACK500 data.")
    zipfile.ZipFile(data_road_zip, 'r').extractall(data_dir)
    kitti_road_dir = os.path.join(data_dir, 'data_road/')

    logging.info("Preparing CRACK500 data.")

    train_txt = "data/trainCRACK500.txt"
    val_txt = "data/valCRACK500.txt"
...
def main():
    """main."""
    with open('../hypes/CRACK500.json', 'r') as f:
        hypes = json.load(f)
